# original code is on GitHub at
# https://github.com/bcdev/geoinfopy/blob/d8fc400b6aa11e145e0f4524191337babfec626a/sandbox/dagmar/QA4EO/aatsr_cloud_shadow_dev.py
...
def cloud_shadow_processor_AASTR():
    ...
    scene_path = "E:\Documents\projects\QA4EO\AATSR4th\\"
    ...
    saa = get_band_or_tiePointGrid(product, 'solar_azimuth_tn')
    oza = get_band_or_tiePointGrid(product, 'sat_zenith_tn')
    x_tx = get_band_or_tiePointGrid(product, 'x_tx') # at nadir line x_tx changes sign. x<272 : x_tx<0; x>=272 : x_tx >0
    ...
    # im = ax[0].imshow(saa-orientation)
    # # ax[0].colorbar()
    # fig.colorbar(im, ax=ax[0], shrink=0.5)
    # ax[0].set_title('sun azimuth - North')
    # im=ax[1].imshow(oaa_correct - orientation)
    # # ax[1].colorbar()
    # fig.colorbar(im, ax=ax[1], shrink=0.5)
    # ax[1].set_title('view azimuth - North')
    # plt.show()

    plt.imshow(orientation)
    plt.colorbar()
    plt.title('North direction [degree]')
    plt.show()

### cloud mask
    cloud_in = get_band_or_tiePointGrid(product, 'cloud_in', 'int32')
    cloudMask = set_cloud_mask_from_cloudBand(cloud_in)

    plt.imshow(cloudMask)
    plt.title('AATSR cloud mask')
    plt.show()

    elevation = get_band_or_tiePointGrid(product, 'elevation_in')
    ...
    for i in range(height):
        for j in range(width):
            if cloudMask[i, j]:
                ...
                ID = np.logical_and(np.logical_and(IndexArray[:, 0] >= 0, IndexArray[:, 0] < width),
                                    np.logical_and(IndexArray[:, 1] >= 0, IndexArray[:, 1] < height))
    ...
    outpath = scene_path + filename[:42] + "_testCloudShadow.dim"
    outProduct = Product('AASTR_cloudShadow', 'AASTR_cloudShadow', width, height)
    ...
    subset_def.setRegion(0, 0, width, height)
    product.transferGeoCodingTo(outProduct, subset_def)

    rad = get_band_or_tiePointGrid(product, 'S2_radiance_in')
