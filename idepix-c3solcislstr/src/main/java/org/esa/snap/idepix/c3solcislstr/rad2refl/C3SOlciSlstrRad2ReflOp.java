 ...
 * @author Marco Peters
 */
@OperatorMetadata(alias = "C3SOlciSlstrRad2Refl",
...
public class C3SOlciSlstrRad2ReflOp extends Operator {

    @Parameter(defaultValue = "OLCI", description = "The sensor", valueSet = {"MERIS", "OLCI", "SLSTR_500m", "C3S_SYN_SLSTR"})
    ...
    public void initialize() throws OperatorException {
        ...
        if (sensor == Sensor.OLCI && spectralBandsAvailable) {
        ...
        } else if (sensor == Sensor.C3S_SYN_SLSTR) {
            slstrSolarFluxMap = SlstrRadReflConverter.getSolarFluxMapFromC3SSYNMetadata(sourceProduct,
                                                                                                                         spectralInputBandNames, isRadToReflMode());
            slstrInvalidImages = SlstrRadReflConverter.
                    createInvalidImages(sourceProduct, spectralInputBandNames, isRadToReflMode());
            converter = new SlstrRadReflConverter(conversionMode);
        } else {
    ...
    private void setInputAndOutputBands() {
        if (sensor == Sensor.SLSTR_500m || sensor == Sensor.C3S_SYN_SLSTR) {
            setInputOutputBandNameLists();
        } else {
    ...
    private void setInputOutputBandNameLists() {
    ...
    private Tile[] getSzaSourceTiles(Rectangle rectangle) {
        try {
            ...
            } else if (sensor.equals(Sensor.C3S_SYN_SLSTR)) {
                // C3S_SYN_SLSTR
                final RasterDataNode szaTpgN = sourceProduct.getRasterDataNode(sensor.getSzaBandNames()[0]);
                if (szaTpgN == null) {
                    throw new OperatorException("No Sun Zenith Angle available from source product - cannot proceed.");
                }
                return new Tile[]{getSourceTile(szaTpgN, rectangle)};
            } else {
                // SLSTR
                final RasterDataNode szaTpgN = sourceProduct.getRasterDataNode(sensor.getSzaBandNames()[0]);
...
//        targetBand.setScalingFactor(1);
    ...
    public static class Spi extends OperatorSpi {
        ...
        public Spi() {
            super(C3SOlciSlstrRad2ReflOp.class);
        }
    }
}
