 ...
 * @author olafd
 */
class OlciSlstrCloudShadowFronts {
    ...
    OlciSlstrCloudShadowFronts(GeoCoding geoCoding,
...
//        final double saaRad = Math.toRadians(saa);
        ...
        final double saaApparent = OlciSlstrUtils.computeApparentSaa(sza, saa, oza, oaa);
        ...
        for (PixelPos pathPixel : pathPixels) {
            ...
            if (sourceRectangle.contains(xCurrent, yCurrent)) {
                if (isCloudForShadow(sourceFlagTile, targetTile, xCurrent, yCurrent)) {
                    ...
                    final float cloudHeight = (float) OlciSlstrUtils.getRefinedHeightFromCtp(ctp, slp, temperature);
